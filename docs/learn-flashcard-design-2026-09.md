...
| **非目标** | ① **手写/编辑卡面**（用户自建卡）；② 独立的卡片管理台（列表/搜索/批量删除/导入导出）；③ 卡级算法升级（不引入 SM-2 的 ease factor / 难度系数）；④ F5 第 2 条「高亮与笔记」；⑤ 卡片写入卷面或参与 `mastery`；⑥ 跨资料合并卡组；⑦ 修正 `useLoopStore.submitAnswer` 的 `applyRating` 既有债（见 §13 D6）。 |
