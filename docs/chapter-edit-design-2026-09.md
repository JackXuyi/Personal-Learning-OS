...
> ⚠️ **2026-09-22 实测补注（本表结论不变，只澄清一处读法）**：上表「跨文档组合学习单元/路径」一行
> 的**归属判断仍成立** —— F7-c 涉及新的领域对象，至今未做。
> 但「跨文档」三字**易被读成「完全不能跨文档组织学习内容」，该读法不成立**：
> `LearningGoal.requiredChapterIds` 自 V2 起就是「**跨文档章节集合**」
> （`learning-system-v2-design-2026-09.md:191`）；目标圈选按资料分组渲染（`GoalFormPage`）、
> 载入侧读全库（`goal-util.ts::loadChapterRows` 注释自称「跨文档章行」）、`scopeOf` 按范围裁剪。
> 即 **「在目标里跨文档勾章、组成一个跨文档学习范围」这一能力早已可用**
> —— README 的 `[x] 六类学习目标 + 章节圈定（requiredChapterIds）` 记的正是它。
> F7-c 真正缺的是**独立于目标的、可命名可复用的「学习单元 / 学习路径」实体**（可在资料库侧创建、被多目标引用）。
