...
| 核对项 | 实测 |
|---|---|
| 全部 44 章（= `doc-3a3025f2` 42 章 + `doc-b577e2b8` 2 章，两份资料合计） | 空字符串 keyPoints = **0 条** |
...
| D4 | 44 章 `keyPointRefs` 全为 0 | **记待办，下一轮单独排查**（另一条根因，不混批）→ ✅ 2026-09-20 已排查并修复，见 §八 遗留 1 |
...
C. 存量快照（真实 plos.chapters）：要点共 135 条（2026-09-16 实测），渲染过滤后丢弃 3 条
  章 18「3.5.5 境外园区」：2 → 1 条，丢弃 ["4."]
  章 20「4.2.5 信用卡使用」：2 → 1 条，丢弃 ["4."]
  章 22「4.4.4 建筑成本」：2 → 1 条，丢弃 ["5."]

C′. 2026-09-20 复测同一口径（重新切分过，计数会漂）：44 章 · 要点 **147 条**（140 + 7）
    —— 本节结论（丢弃 3 条）不受影响，受影响的是「135」这个总数。
```
...
| # | 问题 | 证据 |
|---|---|---|
| 1 | ✅ **已修（2026-09-20）**：两份资料共 44 章，`keyPointRefs` **连字段都不存在**（不是空数组）—— 要点分析的 AI 结果从未落库过**任何一章**。归因是「**一轮从未跑完**」，**不是**原记的「AI 要点抽取未返回任何带原文出处的要点」 | ① 真实快照 2026-09-20：`plos.chapters` **44/44** 章均无 `keyPointRefs` 字段；`doc-3a3025f2.analysis` 只有 `chaptersAt={1789455723200}`、**没有 `keyPointsAt`**；② 5 个日志共 12 条 `[ai:analyze]`，`要点分析完成` **仅 1 条**（`doc-418a4116` `ok=2 ms=1163530` —— 2 章跑了 **19.4 分钟**），该文档与 `doc-b577e2b8` 从未出现；③ 原引的那条 `章要点分析失败 doc=doc-3a3025f2 chapter=1.5.1 民族` 只是第 2 章的**偶发单章失败**，与 0 引用无因果。修复方案见 `docs/library-keypoint-persist-design-2026-09.md` |
...
| 2026-09-20 | 遗留 1 排查完毕并修复：**更正归因**（「一轮从未跑完」而非「锚不上」）+ 补实测证据（44/44 章无 `keyPointRefs` 字段、`analysis` 无 `keyPointsAt`、全史仅 1 条「要点分析完成」）；§2.1 补「44 章 = 两份合计」的口径、§6.1 补 2026-09-20 复测计数（135 → 147，随重新切分漂移）。修复方案另立 `docs/library-keypoint-persist-design-2026-09.md` | Agent |
