...
> **2026-09-22 时点注（原行保留不动）**：上表 `#10` / `#11` 两行是**收口前**的快照 ——
>
> - **#10 已收口**：不是「合并 / 删除一个 planner」，而是**语义归位**。实测两者**都活着且服务
>   不同页面**（章级 → 首页 · `/plan` · `QuizReportPage`；概念层 → `/assessment` · 命令面板 ·
>   非概念 `ReviewSession`），是**上下两级**（章级选章 → 概念层选章内概念），不是同一件事的两套实现。
>   已落到三处注释：`learning-planner.ts` 文件头 / `domain/plan.ts` / `useLoopStore.ts`。
> - **#11 部分收口**：原判「零消费」**不完整** —— 真实形态是**两把尺子**：`AssessmentSession`
>   由 `mastery` **重新推导**起始档位（`:49-55`），无视引擎已持久化的 `UnitMastery.cognitiveLevel`。
>   已收口：次序唯一真源 `domain::COGNITIVE_ORDER`，起始档位改读持久化值。
>   **仍开放**：`bandOfMastery` 出卷难度**仍未**引入 `cognitiveLevel` 权重（属产品决策）。
>
> 详见 `docs/tech-debt-closeout-2026-09.md`。
...
2. ~~**P2 #10**（章级与概念层两套 planner 并行）与 **P2 #11**（`cognitiveLevel` 零消费）—— 属**功能改动**，不在本轮回扫范围。~~
   → **2026-09-22 已单独立项收口**：#10 **语义归位**（零算法改动，只改叙述）；#11 **消「两把尺子」**
   （仍开放的是「出卷难度引入 `cognitiveLevel` 权重」那一层）。见 `docs/tech-debt-closeout-2026-09.md`。
...
## 10. roadmap §F7 范围 3：一个「未做」里藏着半件「已做」（2026-09-22，第三轮）

### 10.1 触发

复核「下一步该做什么」时，按 roadmap §零.2 的原句把 **F7 范围 3「跨文档组合」**当作
「学习侧的**结构性短板**」来排期。按 §1 的口径**先实测再排期** —— 结论与读法**相反**。

### 10.2 逐层实测：这一条含两层

| 层 | 旧读法 | 实测 | 判据 |
|---|---|---|---|
| **能力层** | 「未做」 | ✅ **早已交付**（自 V2 设计起即如此） | 下方四级证据链 |
| **实体层** | 「未做」 | ❌ **确实未做** ＝ F7-c | `chapter-edit-design-2026-09.md §2.2` 非目标表：「涉及新的领域对象」 |

**能力层证据链（四级，逐级可验）**：

| 级 | 位置 | 事实 |
|---|---|---|
| 类型 | `learning-system-v2-design-2026-09.md:191` | `requiredChapterIds: string[];  // V2 主维度（跨文档章节集合）` —— **设计之初就是跨文档的** |
| 载入 | `features/goals/goal-util.ts:14,33` | `ChapterRow` 注释「**跨文档**章行（含所属文档标题）」；`loadChapterRows` 遍历 `listDocuments()` 读**全库**章 |
| 交互 | `features/goals/GoalFormPage.tsx:91,140,308-320` | 按 `docTitle` 分组渲染**全部**资料；每份资料独立「全选」（`toggleDoc`）；`selected` 是**全局 Set** |
| 消费 | `goal-util.ts:45-49` → planner / 就绪度 / 复盘 / 能力评测 | `scopeOf` 按范围过滤，下游全部按该范围工作 |

**旁证（决定性）**：`goal-capability-assessment-design-2026-09.md:125` 在 **F6 实施时（2026-09-15）**
就已实测记录「圈选支持**跨文档**（`GoalFormPage` 按文档分组渲染，`selected` 是全局 Set）」，
`G2` 风险条（`:169`）更以「目标范围可跨文档」为前提推演「跨文档干扰项会串味」。

→ **该能力早在 2026-09-15 就存在、且被写进文档了；只是没人回头去改 roadmap / README 的账。**

### 10.3 为什么不能「直接勾上 README」

README 双语有两条容易被读成重复的条目：

| 位置 | 条目 | 状态 |
|---|---|---|
| 🎯 目标与计划 | `[x] 六类学习目标 + 章节圈定（requiredChapterIds）` | 已勾 |
| ✂️ 切分与结构 | `[ ] 跨文档组成学习单元 / 学习路径 P2`（EN `:428` Cross-document study unit / learning path） | 未勾 |

**两条并不重叠** —— 一条是**目标的范围字段**（已有），一条是**独立于目标的学习单元实体**（未有）。
后者正是 `chapter-edit-design §2.2` 归为 **F7-c「涉及新的领域对象」** 的那一项。

⚠️ 若按「能力层已交付」直接把下面那条勾上，就是**把两层压成一层** → 构成
README 层面的**过度声称**（roadmap §四 第 4 条记过同类教训）。

### 10.4 处置

| 文档 | 动作 |
|---|---|
| `roadmap-next-features-plan-2026-09.md` | **换理由**（§零.2 保留原句 + 加实测注，不删句）＋ 候选表 F7 行 / §F7 状态块 / 范围 3 / §三 mermaid + 「无前置」+「部分交付」/ §五 / §附 / 变更记录 —— 共 **10 处** |
| `chapter-edit-design-2026-09.md §2.2` | 表下加**实测补注**：结论不改，只澄清「跨文档」的读法 |
| **README 双语** | ⚠️ **不改**；`[x]` 37 / `[ ]` 9 **不变** |

### 10.5 一条可复用判断（建议并入 skill）

> **「未做的 feature」其描述里出现的能力词，可能已在别处交付了一半。**
>
> 判据：遇到「某能力未做」的断言，除了找**消费方**（§1 原有口径），还要追问一句 ——
> **这句话里有没有一个已经被交付的子能力？**
> 有 ⇒ 该条应**降级为「部分交付」并写清两层**，而不是简单勾上、也不是维持「未做」。
>
