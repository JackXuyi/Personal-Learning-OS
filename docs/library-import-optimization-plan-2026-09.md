...
| 作者 | Agent |
| 日期 | 2026-09-13 |
| 状态 | **已实施（O1~O4，2026-09-13）**；O5 已由 `docs/import-ai-enrich-design-2026-09.md` 另行实施（**其 D2 决策被取代**，见 §2）；O6/O7 未实施 |
...
| **O5** | 🟡 | 导入后分析全手动：概览 / 概念 / 要点都要进详情页逐章点 | `analyze-service.ts` 仅由详情页 Tab 触发；导入结果卡只有「索引入队状态」 | 结果卡增加可选「导入后自动分析」开关（默认关），开启则后台串行跑要点+概览（`onlyMissing` 语义，失败静默）。受 D2 决策约束 —— ⚠️ **该行与 D2 均已被取代（2026-09-14）**：实际落地方案见 `docs/import-ai-enrich-design-2026-09.md`，范围为**资料标题 + 整篇概览**（不含要点 / 概念），触发为**设置页全局开关且默认开** |
...
| **D2** | O5 自动分析 | A. 不做（保持手动）· B. 全局设置开关，默认关 · C. 导入结果卡一次性勾选 | ~~**C**——不新增设置项，按次选择，符合现有结果卡的交互形态~~ ⚠️ **已作废（2026-09-14）**：`docs/import-ai-enrich-design-2026-09.md` 的 D4 终选 **B 变体——设置页全局开关，默认开**（开关只关自动、不关手动），理由是导入即用、批量导入不必逐次勾选 |
...
| 2026-09-14 | O5 由 `docs/import-ai-enrich-design-2026-09.md` 另行实施（资料标题 + 整篇概览，设置页全局开关默认开）；**本方案的 D2 决策（结果卡一次性勾选）就此作废**，O5 行与 D2 行均已标注取代关系 | Agent |
