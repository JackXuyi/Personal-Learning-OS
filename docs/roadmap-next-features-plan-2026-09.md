...
| **P2** ◐ | F8 导入覆盖面 | OCR / DOCX / EPUB / URL，降低第一分钟摩擦（**0 字符检测已实现 2026-09-21**；**DOCX 解析已实现 2026-09-23**；OCR / EPUB / URL 待做） | 增量 |
...
**现状证据**（2026-09-21 实测更正；DOCX 一条于 2026-09-23 再次更正）
...
- ~~`import/` 下无 DOCX / EPUB 解析器 ✅ **实测为真**（目录仅 md / txt / pdf / github 四路；`package.json` 无 mammoth / epub / readability 类依赖）~~
  ⚠️ **本条一半已过时（2026-09-23）**：DOCX 解析已交付（`import/{docx,mammoth-reader,docx-markdown,heading-patterns}.ts`；
  `package.json` 新增 `mammoth`）。**仍成立的是 EPUB 解析器不存在**，以及 `package.json` 仍无 epub / readability 类依赖。
- 无 URL 抓取入口 ✅ **实测为真**
...
3. ~~**DOCX 解析** → 复用既有 md 标题提升逻辑~~ ✅ **已实现（2026-09-23）** —— 方案 `docs/library-import-docx-design-2026-09.md` ·
   ...
   **不靠命名样式**（`pStyle` 全空、`outlineLvl` 全 0），而是靠 `w:sz` 字号 + 粗体 ⇒ 标题判定做成**四层**
   （命名样式 → 数字 styleId → **字号档位** → 编号兜底），只有第四层复用既有 `heading-patterns.ts`。
   非目标（明确不做）：`.doc` / 加密文档（拒绝 + 可行动提示）、行内样式、图片（跳过、**不内联 base64**）、文本框内容。
4. **EPUB 解析**
...
**无前置、可立即开工**：~~F5~~（已实施）· ~~F7~~（范围 1 已落地；范围 2 / 4 与 F7-c 独立实体可续做）· ~~F8~~（范围 1 / 3 已实现，范围 2 / 4 / 5 可续做）
...
**部分交付**：**F5**（四项全落地 2026-09-14～09-16，D1-C/D5-C 可见性待做）· **F7**（范围 1 于 2026-09-14；范围 3 的**能力层**自 V2 起已具备，**实体层**即 F7-c 未做）· **F8**（范围 1 于 2026-09-21；范围 3 DOCX 于 2026-09-23）
...
> 仍未做：**F8 导入覆盖面**（0 字符检测已于 2026-09-21 交付、**DOCX 解析已于 2026-09-23 交付**，仍开放的是 OCR / EPUB / URL 三项）· **F7 范围 2 / 4**（章节拆分 / 前置关系人工调整）+ **F7-c 独立学习单元实体**（跨文档**圈选**自 V2 起已具备，2026-09-22 实测更正）· **F5 两处可见性**（D1-C 章级队列纳入到期卡 / D5-C 首页卡片入口）。
...
- **2026-09-22 补测**：~~「跨文档组合学习单元 / 学习路径」完全未做~~ → **能力层早已交付**（`requiredChapterIds` 自 V2 起即「跨文档章节集合」/ `loadChapterRows` 读全库 / `GoalFormPage` 按资料分组跨文档勾选 / `scopeOf` 按范围裁剪）；真正未做的是**独立领域对象**（F7-c）。⚠️ **README 双语的两条复选框并不重叠**（`[x] 章节圈定` ＝目标范围字段 ／ `[ ] Cross-document study unit / learning path` ＝独立实体），故 **README 不改、计数 37 / 9 不变**（该日基线；**2026-09-23 DOCX 交付后实测为 `[x]` 38 · `[ ]` 8**）。
...
| 2026-09-23 | **F8 范围 3（DOCX 解析）落地并回写本文**：§F8「现状证据」第二条（「`import/` 下无 DOCX 解析器」）划线更正为**一半已过时**（DOCX 已交付、EPUB 仍不存在）；范围 3 标 ✅ 已实现并**更正原设想**（原写「复用既有 md 标题提升逻辑」，实测真实 DOCX 标题靠 `w:sz` **字号**而非编号形态 ⇒ 判据改为四层，仅第四层复用）；§零 候选表 F8 行、§零 推进顺序、§三 mermaid F8 节点、§三「无前置」/「部分交付」两行、§五 总结同步。⚠️ **README 双语同步（`[x]` 38 · `[ ]` 8，顶部统计行手工同步，实测复核）**。⚠️ 本次实施**推翻了方案文档自身的一处写法**（`OLE_MAGIC = 0xd0cf11e0` 是大端读数，与小端比对永不相等 ⇒ 魔数判定静默失效），已改为字节序列比对并由单测锁住 | `docs/library-import-docx-design-2026-09.md` · `docs/library-import-docx-task-runbook-2026-09.md`（T1–T11 done） |
