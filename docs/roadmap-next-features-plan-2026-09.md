...
| **P1** ◐ | F3 学习复盘与趋势 | 把已经记下来的证据流变成可看的成长轨迹（**方案已出 2026-09-15，待确认**，见 `docs/progress-analytics-design-2026-09.md`） | 中 |
...
F1 画像  →  F7 章节编辑（快赢，1~2 天）  →  F4 导出  →  F3 复盘
                                                          ↓
                              F2 时间维度 ✅（2026-09-15）  →  F5 主动学习  →  F6 能力评测 ✅（2026-09-15）
                                                          ↓
                                                     F8 导入（可随时插入）
```
...
### F3 · 学习复盘与趋势（Progress Analytics）　`P1 · 中`　◐ **方案已出（2026-09-15，待确认）**

> **状态**：◐ **方案已产出**（12 章），D1–D4 四项决策定案（全取推荐），见 `docs/progress-analytics-design-2026-09.md`。
> **未写任何生产代码** —— runbook（T1–T9）待方案确认后建立。
> 方案阶段即否决了本节原有的两处前提（**范围第 5 条**与 **Done 标准第 3 条**，见下），
> 结论：**本特性不新增任何 storage 方法**，聚合全部走纯前端派生（`features/progress/analytics.ts`）。
...
- `src/` 下 grep 学习趋势类标识（`heatmap` / 活动 `streak` / `trend`）→ **无任何聚合或趋势代码**
  （唯一命中是答题**连对** `correctStreak`，`features/assessment/AssessmentSession.tsx`，与本特性语义无关）
- → 数据是"已建未用"：有矿，没有冶炼厂
...
5. ~~`StorageAdapter` 补聚合查询~~ —— ❌ **已被方案否决**（前提不成立：`listEvidence()` 在 memory 后端就是内存数组的浅拷贝 + 排序，无 IO；且被 500 条上限封顶）→ 聚合改为**纯前端派生**，不新增任何 storage 方法

**依赖**：无（数据已在）　**被依赖**：F6 —— ⚠️ **F6 已于 2026-09-15 实施**，但其能力报告的「趋势」维度仍缺数据支撑，等本特性补齐

**Done 标准**

- learner 页出现热力图 + 趋势曲线 + 弱点榜三块 —— ⚠️ **方案改档**（D1）：改为**新建独立路由 `/progress`**（`/learner` 已 5 块、语义是「系统怎么理解我」），`/learner` 留入口卡
- 空数据 / 单条数据有合理空态，不出现分不清的空白图表
- ⚠️ ~~新增聚合查询有单测~~（已否决）→ 改为**新增纯函数有单测**；
  ⚠️ ~~1 万条 evidence 下页面响应可接受~~ —— **前提不成立**：`evidenceLog` 有 500 条硬上限（`memory.ts:440` `slice(-500)`，超出即丢最旧），1 万条不可能达到
  → 改为：上限提至 **5000**（`EVIDENCE_LOG_MAX`），热力图标注**实际覆盖区间**
...
**无前置、可立即开工**：F4 · F5 · F7 · F8（~~F1~~ → **已实施 2026-09-14**）
**有前置**：~~F2（← F1）~~ → **F2 已实施（2026-09-15）** · ~~F6（← F1 + F3 + 产品定义澄清）~~ → **F6 已实施（2026-09-15）**
...
> **评测这条腿已闭环，学习这条腿长出一半；计划开始带上时间，还差「看得见」与「带得走」。**
> 已收口：让系统知道你**是谁**（F1 ✅）、让计划带上**时间**（F2 ✅ 2026-09-15）、让你在学习时**能动手**（F5 ◐ —— 提问 / 费曼复述 / 自测卡已做，笔记未做）、回答"**我够格了吗**"（F6 ✅ 2026-09-15）、把章节结构的控制权交给你（F7 ◐ —— 重命名 / 合并 / 拖拽排序已做）。
> 仍未做：让积累的证据**看得见**（F3 · 趋势与复盘）、让你的投入**可携带**（F4 —— README 产品原则 #1 的过度承诺仍欠着）、降低第一分钟摩擦（F8 · 导入覆盖面）。
...
- 复核到的"未实现"项：`LearnerProfile` 不存在、`StorageAdapter` 无导出方法、`deadlineAt` 零消费点（**F1 后已过期，F2 起参与配额摊派**）、章节编辑原语零 UI 调用点（**F7-a 后已过期**）、全仓无 trend/heatmap/streak
