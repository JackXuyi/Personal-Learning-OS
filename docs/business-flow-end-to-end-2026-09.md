...
> ⚠️ **2026-09-22 追加**：#10 已**收口**（语义归位＝两级分工，**不是**合并/删除一个 planner）；
> #11 已**部分收口**（消掉「两把尺子」，出卷难度引入 `cognitiveLevel` 权重**仍开放**）。
> 见 `docs/tech-debt-closeout-2026-09.md`。
...
| ~~10~~ | ~~章级与概念层两套 planner 并行（**仍开放**，2026-09-21 实测确认两者并存）~~ ✅ **已收口（2026-09-22）** —— **语义归位**，不是合并/删除 | ✅ 原建议（明确调用边界）按「**两级分工**」落到三处注释：章级选章 → 概念层选章内概念。实测两者服务**不同页面**（章级 → 首页 · `/plan` · `QuizReportPage`；概念层 → `/assessment` · 命令面板 · 非概念 `ReviewSession`），**不是两个候选方案** |
| ~~11~~ | ~~难度自适应只看 mastery，不看 `cognitiveLevel` 累积~~ **部分收口（2026-09-22）** | ✅ 已消「**两把尺子**」：原判「零消费」**不完整** —— 真实形态是 `AssessmentSession` 由 `mastery` **重新推导**起始档位、无视引擎持久化的 `UnitMastery.cognitiveLevel`。已把次序抽到 `domain::COGNITIVE_ORDER` 唯一真源，起始档位改读持久化值。**仍开放**：出卷难度引入 `cognitiveLevel` 权重、无历史时用 `profile.level` 定 band（产品决策，未拍板） |
...
> **「诊断」类段落同样会过时 —— 引用前先实测。**
>
> ⚠️ **2026-09-22 再追加**：上面这段诊断**自己**也过时了一处 —— **#10 已收口**（语义归位）、
> **#11 已部分收口**（消掉「两把尺子」）。此刻真正仍开放的是：**#11 的出卷难度权重那一层**、
> **达标收口两项**、以及 `roadmap` 的 F7 / F8 余项。见 `docs/tech-debt-closeout-2026-09.md`。
