...
> **时点注（2026-09-22）**：上面这个「不改」是就**本轮的孤儿卷议题**而言的（那批是历史产物，
> 改级联救不回来），**不代表该模块此后不动**。同日为「删资料回收派生学习资产」补齐了
> 6 项同族实体 —— 见 `docs/document-cascade-cleanup-2026-09.md`。**判据没变**：
> 有存量症状 ⇒ 先清存量；机制有漏 ⇒ 才改级联。两件事不要合并成一句「改不改级联」。
...
- **孤儿引用清理策略**：本轮只处理了试卷侧。~~证据流与 `learner.byUnit` 走
  append-only（c 档）；`deleteDocumentCascade` 是否补证据流清理仍待拍板。~~
  → **2026-09-22 拍板并实施**，拆成两条**结论相反**的处置：
  - **证据流**：**维持 append-only，不随资料清**。实测它的三个消费方（热力图逐日计数 /
    F9 记忆的节奏与模式分布 / 首页最近证据）**全都不看主体、只看行为** ⇒ 按主体删行
    等于**回溯改写已发生的事实**。它的孤儿行由**消费侧**兜底（首页跳过解析不到的章级行）。
  - **`learner.byUnit` 及其同族**：**改为随资料回收**。消费方过滤（`analytics.ts:354`）
    是**兜底不是清理** —— 其真源（卷面）已被级联删除，留着键就是把错误数据摊给每个
    消费方各自处理。同批补齐复述 / 卡状态 / Section / 目标悬空章 id / 包溯源，共 6 项。
  - 详见 `docs/document-cascade-cleanup-2026-09.md`（含「剔空则不动」防语义翻转的例外）。
