...
> **v0.4 补（2026-09-22 · 实施期补漏）**：收口后实测发现 §12.2 里的「T17 顺带确认两个页面的异常分支」
> 这一条**没有被执行** —— T17 只落了存储层用例（`tests/storage-documents.test.ts`）与 runbook 回扫。
> 已补：资料库页最小错误态（`f481577`）+ 守卫 `TC-PAGE-01`（`4db6610`）；`HomePage` 经实测**无需改动**。
> ⚠️ 根因值得记：该承诺**只写在 §12.2 的风险表里、没进 §9 任务表** —— 「顺带确认」型承诺最容易漏，
> 与 T17 漏改 `docs/storage-architecture-rag-2026-09.md` 主文档是同一个病。
...
| ✅ **已解除**（2026-09-22 补） | ~~`listDocuments()` 拒绝时页面的表现未经验证~~：实测 `LibraryPage.load()` 原先直接 `await`，被拒后 `docs` 停在 `[]` → 落到 `docs.length === 0` 的「还没有资料」空态，**与真正的「空库」是同一个页面** | **已补**（`f481577`）：`load()` 捕获失败、错误态**排在空态之前**、失败时不显示副标题；用户可见文案走 i18n，技术细节小字给出。实测 **`HomePage` 无需改动**（错误态由 `useLoopStore.refresh` 的 `catch → set({ error })` 承担，而 `runChapterLoop` 会读 `listDocuments()`）。守卫 `TC-PAGE-01` 锁「错误态早于空态」并做过负向验证。⚠️ **真机故障注入（改名 / 只读 `plos.db`）仍是 §12.3 第 11 条的手工项** |
...
11. 🆕 **SQLite 故障时的诚实报错**：临时把 `plos.db` 改名或设为只读后启动 → 资料库**不显示空库**，而是明确的错误提示；恢复文件后重启一切正常。（代码侧已具备：`f481577` 补的资料库页错误态 + 首页既有错误卡片；**本项仍需在桌面端实跑**）
...
| 2026-09-22 | **v0.4 补 · 实施期补漏（§12.2 的 🟠 项关闭）**：实测 T17 只落了存储层用例与 runbook 回扫，**未动**它在 §12.2 承诺确认的两个页面 —— `LibraryPage.load()` 仍是裸 `await`，SQLite 故障时落到「还没有资料」空态（与空库**同一个页面**，用户会以为资料丢了）。已补：① `fb7410d` i18n 文案先行（`learn.library.loadFailedTitle` / `loadFailedDesc`，zh/en 成对）；② `f481577` 资料库页最小错误态（`loadError` state + `load()` try/catch + 错误态**排在空态之前** + 失败时不显示副标题，全部数据就位后才清错）；③ `4db6610` 守卫 `TC-PAGE-01`（包测试 **40 → 41 例**；负向验证过 —— 顺序对调或去掉 `catch` 都会变红）。实测 **`HomePage` 无需改动**（错误态由 `useLoopStore.refresh` 的 `catch → set({ error })` 承担）。⚠️ 根因：该承诺只写在 §12.2 的风险表里、**没进 §9 任务表** —— 与 T17 漏改 `docs/storage-architecture-rag-2026-09.md` 主文档**同一个病**，「顺带确认」型承诺必漏 | `src/features/learn/LibraryPage.tsx` · `src/i18n/messages/zh.ts` · `src/i18n/messages/en.ts` · `tests/knowledge-pack.test.ts` |
