...
3. **存在任一待澄清项时，必须先向用户确认**，不得自行假设产品或安全边界。
   - 优先使用 `AskUserQuestion` 给出具体选项（一次 1–4 题，每题 2–4 个选项，推荐项放首位并在 label 标注「推荐」）
...
- [ ] 所有待澄清项已向用户确认 —— **并在方案 §3.4「决策点（D1–Dn）」逐条登记选项 / 推荐档 / 理由**（改档后全量回扫正文）
- [ ] 12 个章节均已填写或标注 N/A
- [ ] 交互流程与用例、测试用例可相互引用
- [ ] 每个改动文件有伪代码
- [ ] 线框覆盖主要状态（默认、空、错、加载）
...
- [ ] 用户尚未确认前未写生产代码
- [ ] **伪代码量纲口径一致**：同一段逻辑里多处估算/换算必须共用**同一个真源与参数**（反例 F2：装填用 `estimateEtaMin` 的默认 pace，而总量来自按画像取 pace 的 `estimatePlanEta` → 成了两把尺子）；**同一行 UI 文案里的多个数字必须同口径**（反例 F2：项数取「不含主行动」的清单、分钟数取「含主行动」的配额组 → 同屏自相矛盾）
- [ ] **时间基准唯一**：`now` / `Date.now()` 只允许在入口注入一次，下游（含 UI 展示）一律复用该值，不得自行再取一次 —— 跨时刻/跨零点会让同一屏的两个数字互相矛盾
- [ ] **测试用例的输入与期望值自洽**：写进表格前**手算一遍**（反例 F2：`TC-UC01-03` 写「队列 80 分钟 + `dueTodayMin=30`」，可 80÷3=27 根本算不出 30）——输入凑不出期望时，改输入或改期望，别让实施者去猜
- [ ] **不凭记忆引用行数/计数**：方案里引用的「当前 N 行」「改后应为 X/Y」会被并行会话的提交作废 —— 收尾一律实测复核（`wc -l` / `grep -c`），并把实测值写回文档。⚠️ **同一条规则适用于提交信息**：`feat:` / `docs:` 正文里写的行数、提交条数、测试条数同样是交付物的一部分，提交前实测（反例：F9 方案归档时先按编辑前的旧计数写「1937 行」，实测已是 1939 行）
