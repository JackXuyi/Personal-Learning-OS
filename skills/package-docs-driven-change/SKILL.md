...
**最常越界的一类：把正则写成了代码的复印件** —— 锚定实参名、局部变量名、参数形参名。
实测：断言「汇总主句按 `kind` 分派」时写成 `/summary\.kind\s*===/`，而分派器的形参叫 `s`
（`summaryHeadline(s)`）→ 断言红、但**不变量成立**。

判别法：**断言失败时先问「是名字变了，还是规则破了」。名字变了 → 改断言（放宽到语义）；规则破了 → 改代码。**
上例的正确写法是取分派器块，断言块内 `\.kind\s*===\s*['"]points['"]` 且两个文案键都在块内
（样件 `tests/keypoint-persist.test.ts::TC-KP-WIRE-08`）。
