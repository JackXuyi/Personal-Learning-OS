...
- [x] 学习者记忆 —— 系统整理出的一份关于你的 markdown 文档（活跃时段 / 复习遵守度 / 学习方式 / 输出习惯），你可以直接改，并会一并交给出题、章内提问与复述批改，用来调整举例与讲解深浅。合并是**行级**的：**你改写过的行永不被覆盖，你删掉的行永不复活**。确定性通道零 AI 本地运行；AI 通道手动触发，发送前把即将离开本机的掩码原文摊开给你看，不点「写进文档」就什么都不写。桌面版还会把它镜像到 `<app_data>/memory/learner-memory.md`，可用你自己的编辑器直接改
