...
- [x] 本地优先的桌面基座 —— Tauri 2、React 19、SQLite
- [x] 多来源导入 —— PDF / Markdown / TXT / 粘贴 / GitHub
- [x] 确定性章节切分 + 可选 AI 精修
- [x] 章节阅读器：AI 要点卡 + 概念图谱
- [x] 出卷引擎 —— 4 种卷型、难度自适应
- [x] 判卷 —— 客观本地判 + 主观 AI 批改与批语
- [x] 报告页：逐章掌握度与前后对比
- [x] 掌握度模型：平滑更新 + 遗忘曲线
- [x] 间隔重复调度（`nextReviewAt` → 复习队列）
- [x] 目标圈定、六级优先学习计划、就绪度
- [x] 混合检索 —— FTS5 + 向量，RRF 融合
- [x] 内置本地模型，离线推理

### 进行中

- [ ] **学习者画像输入** —— 自评水平、每周可用时间、学习偏好
      （目前画像页是只读的，画像尚未影响计划与出题难度）
- [ ] **AI 评测契约** —— `generateAssessment` / `evaluateAnswer` 仍是占位；
      目标级能力评测尚未接线
- [ ] 章节人工微调 —— 重命名 / 合并 / 重排（引擎原语已存在，UI 未接线）
- [ ] 截止期感知的排程（当前 ETA 不考虑 `deadlineAt`）
- [ ] 扫描件 PDF 的 OCR 兜底（纯图片 PDF 经 pdfjs 解析为 0 字符）
