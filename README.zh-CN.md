...
- [x] 划线高亮与笔记 —— 选中原文即划线，可加一句笔记；右栏「我的划线」点回原文并闪烁定位（**零 AI · 不写证据流 · 不动掌握度**；重切分/合并章节后自动重新定位）
...
- [x] 能力项清单 —— AI 从目标与圈定章节提炼 3–6 个能力项，也可手工增删改；每项的达标阈值可调
...
- [x] 计划的时间维度 —— 设了截止日时给出「今天建议 N 分钟 · M 项」，队列按今天 / 本周 / 之后分组；落后时首页显式提示，**不改动规划优先级**
- [x] 复盘与趋势 —— 活动热力图（按天聚合，显式标注实际记录区间与裁剪）、掌握度趋势曲线（判卷时点快照，可下钻到单章）、弱点排行（掌握度缺口 + 卷面错误 + 误解数三因子）、目标进度时间线（对照截止日）；全部为**纯只读派生**，不动掌握度、不写任何状态
