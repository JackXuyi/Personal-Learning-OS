...
export const en: Messages = {
  ...
  settings: {
    ...
    storage: {
      ...
      data: {
        ...
        /** Community knowledge packs (F10; docs/community-knowledge-pack-design-2026-09.md). */
        pack: {
          title: "Community knowledge pack",
          desc: "Bundle selected documents into a single .ploskp.json you can share anywhere; the other side imports it into their own library. A pack carries content and structure only (documents / chapters / sections / chunks / concepts) — never study progress or notes.",
          notice: "A pack is plain JSON and carries your attribution and license statement; please make sure the selected documents are shareable. Local paths and goal links are stripped automatically.",
          exportTitle: "Export a knowledge pack",
          exportPick: "Select documents to bundle",
          exportNoDocs: "No documents to bundle yet.",
          exportSelected: (n: number) => `${n} selected`,
          exportClear: "Clear selection",
          titleLabel: "Pack title",
          titlePlaceholder: (n: number) => `${n} document(s)`,
          descLabel: "Description (optional)",
          descPlaceholder: "Say a few words about this pack…",
          authorLabel: "Credit (optional)",
          ...
          licenseNone: "Not stated",
          export: "Export pack",
          exporting: "Collecting content…",
          exportDonePath: (p: string) => `Saved to ${p}`,
          exportDoneDownload: "Pack downloaded — check your browser's download folder.",
          exportSize: (b: string) => `Pack size ≈ ${b}`,
          warnTooLarge: "The pack exceeds the local writable capacity; the other side may not fit it.",
          warnNoBody: (n: number) => `${n} document(s) have no text snapshot, so the other side cannot read the source.`,
          warnNoChunks: (n: number) => `${n} document(s) have no chunks, so they will not be searchable after import.`,
          warnOrphanRelations: (n: number) => `${n} concept relation(s) were left out (missing endpoint).`,
          warnOrphanUnits: (n: number) => `${n} concept(s) point outside the pack and were left out.`,
          importTitle: "Import a knowledge pack",
          importHint: "Import is additive: nothing on this machine is cleared or overwritten, and the author's study progress is never imported.",
          importPick: "Choose a pack file…",
          importUrlLabel: "Or import from a link",
          importUrlPlaceholder: "https://…",
          importFetch: "Fetch",
          fetching: "Fetching…",
          importing: "Writing data…",
          previewTitle: "Pack contents",
          previewAuthor: (a: string) => `by ${a}`,
          previewAiDerived: "Contains AI-derived content (overview / key points / concepts)",
          previewAlreadyImported: "This pack has been imported before.",
          import: "Import",
          importDone: (title: string) => `Imported “${title}”`,
          stats: (s: string) => `Written: ${s}`,
          statsForeignProgress: (n: number) => `Discarded ${n} study-progress record(s) from the author.`,
          statsDangling: (n: number) => `Cleared ${n} dangling reference(s).`,
          statsRelationsDropped: (n: number) => `Dropped ${n} dangling relation(s).`,
          importedTitle: "Imported packs",
          importedEmpty: "No packs imported yet.",
          importedGone: "The documents from this pack were deleted.",
          importedOpen: "View documents",
          importedRemove: "Remove record",
          errNotJson: "The file is not valid JSON.",
          errNotPack: "This is not a PLOS knowledge pack.",
          errVersionNewer: "This pack was exported by a newer version. Please update the app first.",
          errUnsupportedVersion: "This pack format is no longer supported.",
          errCorrupt: "The pack is incomplete or has been modified — nothing was changed.",
          errTooLarge: "The file exceeds the 100 MB limit and was not read.",
          errTooLargeForStore: "This machine cannot fit the pack — nothing was written (the desktop build has no such limit).",
          errEmpty: "This pack has nothing to import.",
          errWriteFailed: "Import did not finish. Please try again.",
          errQuota: "Not enough browser-preview storage. Use the desktop build to import a pack.",
          errInvalidUrl: "Invalid link: only https links are supported.",
          errNetwork: "Fetch failed: the network is unavailable or the link no longer works.",
          errUnknown: "The operation failed. Please try again.",
        },
      },
    },
  ...
  knowledge: {
    ...
    chapterGraph: {
      ...
      hint: "Concepts are extracted by AI from the chapter body; self-rated reviews only refresh review scheduling (no papers at concept level — mastery comes from assessments). Click a node to focus · double-click for the chapter source · gap concepts (<80%) pulse indigo.",
  ...
  review: {
    ...
    conceptDoneSubtitle:
      "No papers at concept level — self-rating only affects review scheduling and confidence; mastery still comes from assessments.",
