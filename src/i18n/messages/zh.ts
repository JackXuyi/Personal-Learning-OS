...
export const zh = {
  ...
  settings: {
    ...
    storage: {
      ...
      data: {
        ...
        /** 社区知识包（F10；docs/community-knowledge-pack-design-2026-09.md）。 */
        pack: {
          title: "社区知识包",
          desc: "把选中的资料打包成一份 .ploskp.json，可在任意渠道分享；对方在自己的库里导入即可。包只含内容与结构（资料 / 章节 / 小节 / 切片 / 概念），不含对方的学习进度与笔记。",
          notice: "知识包是明文 JSON，且会带上你的署名与许可声明；请确认选中的资料可以分享。本机路径与目标关联已自动剥离。",
          exportTitle: "导出知识包",
          exportPick: "选择要打包的资料",
          exportNoDocs: "还没有可打包的资料。",
          exportSelected: (n: number) => `已选 ${n} 份`,
          exportClear: "清空选择",
          titleLabel: "包标题",
          titlePlaceholder: (n: number) => `${n} 份资料`,
          descLabel: "说明（可选）",
          descPlaceholder: "给这份包写一句介绍…",
          authorLabel: "署名（可选）",
          licenseLabel: "许可",
          licenses: {
            cc0: "CC0（公有领域）",
          ...
          exporting: "正在收集内容…",
          exportDonePath: (p: string) => `已保存到 ${p}`,
          exportDoneDownload: "已下载知识包文件，请留意浏览器的下载目录。",
          exportSize: (b: string) => `包体积约 ${b}`,
          warnTooLarge: "包体积超过本机可写入容量，导入方可能装不下。",
          warnNoBody: (n: number) => `${n} 份资料没有正文快照，对方看不到原文。`,
          warnNoChunks: (n: number) => `${n} 份资料没有切片，对方导入后无法检索。`,
          warnOrphanRelations: (n: number) => `${n} 条概念关系因端点缺失未进包。`,
          warnOrphanUnits: (n: number) => `${n} 个概念指向包外资料，未进包。`,
          importTitle: "导入知识包",
          importHint: "导入只做加法：本机已有内容不会被清空或覆盖；原作者的学习进度一律不导入。",
          importPick: "选择知识包文件…",
          importUrlLabel: "或从链接导入",
          importUrlPlaceholder: "https://…",
          importFetch: "拉取",
          fetching: "正在拉取…",
          importing: "正在写入数据…",
          previewTitle: "包内容",
          previewAuthor: (a: string) => `来自 ${a}`,
          previewAiDerived: "含 AI 派生内容（概览 / 要点 / 概念）",
          previewAlreadyImported: "这份包之前已经导入过。",
          import: "导入",
          importDone: (title: string) => `已导入《${title}》`,
          stats: (s: string) => `写入：${s}`,
          statsForeignProgress: (n: number) => `已丢弃原作者的学习状态 ${n} 条。`,
          statsDangling: (n: number) => `已清理失效引用 ${n} 处。`,
          statsRelationsDropped: (n: number) => `已丢弃悬空关系 ${n} 条。`,
          importedTitle: "已导入的知识包",
          importedEmpty: "还没有导入过任何知识包。",
          importedGone: "该包导入的资料已删除。",
          importedOpen: "查看资料",
          importedRemove: "移除记录",
          errNotJson: "文件不是合法 JSON。",
          errNotPack: "这不是 PLOS 知识包文件。",
          errVersionNewer: "该知识包由更新版本导出，请先升级应用。",
          errUnsupportedVersion: "该知识包的格式已不再支持。",
          errCorrupt: "知识包内容不完整或已被修改，未改动任何数据。",
          errTooLarge: "文件超过 100 MB 上限，已拒绝读取。",
          errTooLargeForStore: "本机可用容量装不下这份包，未写入任何数据（桌面端不受此限）。",
          errEmpty: "这份包里没有可导入的内容。",
          errWriteFailed: "导入未完成，请重试。",
          errQuota: "浏览器预览容量不足，请改用桌面端导入知识包。",
          errInvalidUrl: "链接无效：只支持 https 链接。",
          errNetwork: "拉取失败：网络不可用或链接已失效。",
          errUnknown: "操作失败，请稍后重试。",
        },
      },
    },
  ...
  knowledge: {
    ...
    chapterGraph: {
      ...
      hint: "概念由 AI 从本章正文提炼，自评只刷新复习调度（概念层无卷面，掌握度由测评决定）。单击节点聚焦 · 双击看本章原文 · 缺口概念（<80%）带 indigo 脉冲。",
  ...
  review: {
    ...
    doneTitle: "本次复习完成 🎉",
    conceptDoneSubtitle: "概念层无卷面——自评只影响复习调度与置信度，掌握度仍由测评决定。",
