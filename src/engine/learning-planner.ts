/**
 * 学习规划器 —— **两级分工**：章级 `buildChapterPlan`（选章）+ 概念层 `buildPlan`（选章内概念）。
 *
 * ⚠️ 2026-09-22 语义归位：本文件曾把概念层写成「Phase 0 **基座**」——读起来像一份
 * 「待被 V2 取代的过渡层」，于是 #10「两套 planner 并行」被当成在「合并 / 删除」里
 * 二选一的债。实测两者**都活着、且服务不同页面**，是**上下两级**而非两个候选方案：
 *
 *   - 章级 `buildChapterPlan`：`loop.ts::runChapterLoop` → 首页「今日主行动」、
 *     `/plan` 计划页、`QuizReportPage` 的「生成学习计划」。回答「**先动哪一章**」。
 *   - 概念层 `buildPlan`：`loop.ts::runLearningLoop` → `/assessment`（推荐下一个待测
 *     概念）、非概念模式的 `ReviewSession`、`CommandPalette`；另由 `ReviewSession`
 *     的概念模式**按章裁剪**后调用（`requiredUnitIds = 该章 unitIds`）。回答
 *     「**章内先动哪个概念**」。
 *
 * 即：**章级选章 → 概念层在其范围内选概念**。这不是「两把尺子」——那条判据是
 * **同一输入算出同一件事**；此处两者的输入与粒度都不同。
 *
 * 概念层：目标 → 缺口 → 有序动作。依赖优先：只有先决缺口被清除后，某缺口单元才可
 * 执行；瓶颈先决条件自然成为首要动作。每个动作都携带其背后的理由（可解释原则）。
 *
 * 章级（docs §5.5 / P7）：章集合 + 章掌握度 → 状态机排序动作。队列顺序遵循
 * 「重学弱章 > 补考弱章 > 复习要点 > 测已学章 > 到期复习(防遗忘) > 推进下一未学章」；
